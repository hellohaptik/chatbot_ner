...
def generate_newman_data(entities_data):
    ...
    for i in range(number_of_records):
        ...
        for k in entities_data:
            entities_list.append(entities_data[k][i])
...
entities = [
    'city', 'budget', 'date', 'number', 'phoneV1',
    'email', 'regex', 'pnr', 'time_range', 'time',
    'person_name'
]
...
if check_if_data_valid(entities):
    try:
        ...
        with open('newman_v1_data.json', 'w') as fp:
            json.dump(newman_data, fp, indent=4)
        os.system('newman run ner_v1_collection.json -d newman_v1_data.json -e dev.json')
    except Exception as e:
        print(str(e))
