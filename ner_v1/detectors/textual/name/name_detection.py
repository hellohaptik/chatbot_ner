...
class NameDetector(object):
    ...
    def detect_hindi_name(self):
        ...
        if not entity_value:
            english_present_regex = re.compile(r'[a-zA-Z\s]+', re.U)
            if english_present_regex.search(text_before_hindi_regex_operations):
                remove_everything_except_english = re.compile(r'[^a-zA-Z\s]+', re.U)
    ...
    def remove_emojis(self, text):
        ...
        """
        emoji_pattern = re.compile(r'[{0}]+'.format(''.join(list(EMOJI_RANGES.values()))), re.UNICODE)
