from ner_v1.detectors.temporal.constant import TAG_PREV, TAG_NEXT, DATE_CONSTANT_FILE, DATETIME_CONSTANT_FILE, \
    NUMERALS_CONSTANT_FILE, TIME_CONSTANT_FILE, TAGGED_DATE, TAGGED_TIME
from ner_v1.detectors.temporal.utils import get_tuple_list
...
class HindiDateTimeTagger(object):
    ...
    def __init__(self, data_directory_path):
        ...
        Args:
            data_directory_path (str): path where data file of language is located
        ...
        self.time_constant_dict = get_tuple_list(data_directory_path.rstrip('/') + '/' + TIME_CONSTANT_FILE)
    ...
    def _update(self, word, method=None):
        ...
        if word in self.date_constant_dict:
            if method == TAG_NEXT:
        ...
        elif word in self.time_constant_dict:
            if method == TAG_NEXT:
                ref_next = " ".join(self.ref_next)
        ...
        elif word in self.datetime_constant_dict:
            if self.datetime_constant_dict[word][0] == 1:
                self.ref_next.append(word)
            else:
                self.ref_prev.append(word)

        elif word in self.numerals_constant_dict or (
                len(word) > 2 and word[:2] in self.numerals_constant_dict or word[:1] in self.numerals_constant_dict):
            self.ref_next.append(word)
    ...
    def _update_original_text(self, word, is_date, is_time, method=None):
        ...
        if word in self.datetime_constant_dict:
            if method == TAG_NEXT:
        ...
        elif word in self.numerals_constant_dict:
                self.original_text_ref_next.append(word)

        elif len(word) > 2 and (word[:2] in self.numerals_constant_dict or word[:1] in self.numerals_constant_dict):
                self.original_text_ref_next.append(word)
    ...
    def get_datetime_tagged(self, message):
        """
